...
def exactDiag(n,j,h):
	...
	sum1 = np.sum([np.array(reduce(np.kron, [id for k in range(i-1)]+[X,X]+[id for k in range(n-i-1)])) for i in range(1,n)],axis = 0)
	sum2 = np.sum([np.array(reduce(np.kron, [id for k in range(i-1)]+[Z]+[id for k in range(n-i)])) for i in range(1,n+1)],axis = 0)
