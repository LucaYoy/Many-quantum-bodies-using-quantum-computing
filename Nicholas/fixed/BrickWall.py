...
class Circuit:
    ...
    def optimize_circuit(self, max_iterations, min_overlap_change, plot=True, show_overlap=True, stopped_flag=False, optimizationnew=False):
        ...
        for obj in self.gates:
            arr = np.array(obj)
            arr = arr.reshape(4,4)
            flattened_gates += arr.flatten().tolist()
            #arr = arr.flatten()
            #flattened_gates.append(arr)
            
        #final_gates = []
        #for i in range(0, len(flattened_gates), 16):
            #final_gates.append(flattened_gates[i:i+16])
        gatesfloat = []
        for gate in flattened_gates:
            gatesfloat.append(float(gate.real))
            gatesfloat.append(float(gate.imag))
            
                              
        return relative_errors, overlaps, final_psi, iterations, relative_overlap_changes, flattened_gates#gatesfloat#flattened_gates#final_gates
